...
def smooth(path, weight_data = 0.1, weight_smooth = 0.1, tolerance = 0.00001):
    ...
    while(ave_error > tolerance):
        ...
        for i in range(len(path)) :
            ...
            # if i==0 :
            #     newpath[0][0] = newpath[(i-1)%len(path)][0]
            #     newpath[0][1] = newpath[(i-1)%len(path)][1]
            # elif i == len(path)-1:
            #     newpath[0][0] = newpath[i][0]
            #     newpath[0][1] = newpath[i][1]

            newpath[i][0] = newpath[i][0] + alpha*(path[i][0]-newpath[i][0]) + beta*(newpath[(i-1)%len(path)][0]-2*newpath[i][0]+newpath[(i+1)%len(path)][0])
            newpath[i][1] = newpath[i][1] + alpha*(path[i][1]-newpath[i][1]) + beta*(newpath[(i-1)%len(path)][1]-2*newpath[i][1]+newpath[(i+1)%len(path)][1])
